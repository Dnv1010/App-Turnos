...
interface TurnoRow {
  ...
  notes: string | null;
  user: { fullName: string; zone: string };
}
...
export default function CoordinadorTurnosPage() {
  ...
  const loadTurnos = useCallback(async () => {
    ...
    try {
      ...
      list = list.filter((t) => !t.notes?.startsWith("Cancelado"));
      list.sort((a, b) => {
        const aAbierto = !a.clockOutAt ? 0 : 1;
        const bAbierto = !b.clockOutAt ? 0 : 1;
  ...
  useEffect(() => {
    if (!profile?.zone) return;
    fetch(`/api/usuarios?zona=${profile?.zone}&role=TECNICO`)
  ...
  function openEditModal(t: TurnoRow) {
    setEditingTurno(t);
    const start = new Date(t.clockInAt);
    const end = t.clockOutAt ? new Date(t.clockOutAt) : null;
    ...
    setEditForm({
      ...
      notes: t.notes?.replace(/\s*\[Editado.*\]$/, "") || "",
    });
  }
  ...
  async function saveTurnoEdit() {
    ...
    try {
      ...
      const res = await fetch(`/api/turnos/${editingTurno.id}`, {
        ...
        body: JSON.stringify({ clockInAt: startISO, clockOutAt: endISO, notes: editForm.notes || undefined }),
      });
      const data = await parseResponseJson<{ ok?: boolean; error?: string; turno?: TurnoRow }>(res);
      if (res.ok && data?.ok) {
        toast.success("Turno actualizado", `Ord: ${data.turno?.regularHours ?? 0}h, HE: ${((data.turno?.daytimeOvertimeHours ?? 0) + (data.turno?.nighttimeOvertimeHours ?? 0)).toFixed(2)}h`);
  ...
  const totalHoras = (t: TurnoRow) => {
    if (!t.clockOutAt) return null;
    return ((new Date(t.clockOutAt).getTime() - new Date(t.clockInAt).getTime()) / (1000 * 60 * 60)).toFixed(2);
  };
  const totalHE = (t: TurnoRow) => (t.daytimeOvertimeHours + t.nighttimeOvertimeHours + t.sundayOvertimeHours + t.nightSundayOvertimeHours).toFixed(2);
  const totalRec = (t: TurnoRow) => (t.nightSurchargeHours + t.sundaySurchargeHours + t.nightSundaySurchargeHours).toFixed(2);

  const columns = [
    { key: "user", label: "Operador", render: (t: TurnoRow) => t.user?.fullName ?? "—" },
    {
      key: "estadoTurno", label: "Estado",
      render: (t: TurnoRow) => t.clockOutAt
    ...
    { key: "clockInLat", label: "Ubicación inicio", render: (t: TurnoRow) => t.clockInLat != null && t.clockInLng != null ? <a href={`https://www.google.com/maps?q=${t.clockInLat},${t.clockInLng}`} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline text-xs flex items-center gap-1"><HiLocationMarker className="h-3.5 w-3.5" />Mapa</a> : "—" },
    { key: "clockOutLat", label: "Ubicación fin", render: (t: TurnoRow) => t.clockOutLat != null && t.clockOutLng != null ? <a href={`https://www.google.com/maps?q=${t.clockOutLat},${t.clockOutLng}`} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline text-xs flex items-center gap-1"><HiLocationMarker className="h-3.5 w-3.5" />Mapa</a> : "—" },
    ...
    { key: "endPhotoUrl", label: "Foto fin", render: (t: TurnoRow) => t.endPhotoUrl ? <a href={t.endPhotoUrl} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline text-xs flex items-center gap-1"><HiPhotograph className="h-3.5 w-3.5" />Ver</a> : "—" },
    {
      ...
      render: (t: TurnoRow) => (
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setConfirmDelete({ id: t.id, fullName: t.user?.fullName ?? "" })} className="text-red-600 hover:text-red-800 p-1" title="Cancelar turno"><HiTrash className="h-4 w-4" /></button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-500 dark:text-[#A0AEC0]">
        Zona {profile?.zone ? getZonaLabel(profile?.zone) : ""} — Editar o cancelar turnos de operadores de tu zona.
      </p>
          ...
          <div className="bg-white dark:bg-[#1A2340] rounded-2xl shadow-2xl dark:shadow-black/40 w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-[#3A4565]">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-[#3A4565]">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-[#A0AEC0]">{editingTurno.user?.fullName} — {formatFechaTurnoDdMmmYyyy(editingTurno.date)}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gray-50 dark:bg-[#0F1629] rounded-lg p-3 text-sm border border-gray-100 dark:border-[#2A3555]">
                ...
                <p className="font-medium text-gray-900 dark:text-white">
                  {new Date(editingTurno.clockInAt).toLocaleTimeString("es-CO", { timeZone: "America/Bogota", hour: "2-digit", minute: "2-digit" })} → {editingTurno.clockOutAt ? new Date(editingTurno.clockOutAt).toLocaleTimeString("es-CO", { timeZone: "America/Bogota", hour: "2-digit", minute: "2-digit" }) : "En curso"}
                </p>
              ...
              <div className="flex gap-3 flex-wrap">
                {editingTurno.clockInLat != null && <a href={`https://www.google.com/maps?q=${editingTurno.clockInLat},${editingTurno.clockInLng}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-blue-600 hover:underline"><HiLocationMarker className="w-3 h-3" /> Ubicación inicio</a>}
                {editingTurno.clockOutLat != null && <a href={`https://www.google.com/maps?q=${editingTurno.clockOutLat},${editingTurno.clockOutLng}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-blue-600 hover:underline"><HiLocationMarker className="w-3 h-3" /> Ubicación fin</a>}
          ...
          <div className="bg-white dark:bg-[#1A2340] rounded-2xl shadow-2xl dark:shadow-black/40 w-full max-w-sm p-6 border border-gray-200 dark:border-[#3A4565]">
            <div className="text-center">
              ...
              <p className="text-gray-500 dark:text-[#A0AEC0] text-sm mb-6">
                Vas a cancelar el turno de <strong>{confirmDelete.fullName}</strong>. Esta acción quedará registrada.
              </p>
