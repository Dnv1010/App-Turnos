...
export default function SupplyTurnosPage() {
  ...
  const totalHoras = (t: TurnoRow) => {
    ...
    const h = (new Date(t.clockOutAt).getTime() - new Date(t.clockInAt).getTime()) / (1000 * 60 * 60);
    return h.toFixed(2);
  };
    ...
    { key: "user", label: "Operador", render: (t: TurnoRow) => t.user?.fullName ?? "—" },
    {
      ...
      render: (t: TurnoRow) => (
        <span className="text-xs px-1.5 py-0.5 rounded font-medium bg-violet-100 text-violet-800 dark:bg-violet-900/40 dark:text-violet-300">
          {getZonaLabel(t.user?.zone || "BOGOTA")}
        </span>
      ),
    },
    {
          ...
          <div className="bg-white dark:bg-[#1A2340] rounded-2xl shadow-2xl dark:shadow-black/40 w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-[#3A4565]">
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gray-50 dark:bg-[#0F1629] rounded-lg p-3 text-sm border border-gray-100 dark:border-[#2A3555]">
                ...
                <p className="font-medium text-gray-900 dark:text-white">{new Date(editingTurno.clockInAt).toLocaleTimeString("es-CO", { timeZone: "America/Bogota", hour: "2-digit", minute: "2-digit" })} → {editingTurno.clockOutAt ? new Date(editingTurno.clockOutAt).toLocaleTimeString("es-CO", { timeZone: "America/Bogota", hour: "2-digit", minute: "2-digit" }) : "En curso"}</p>
              ...
              <div className="flex gap-3 flex-wrap">
                {editingTurno.clockInLat != null && (
                  <a href={`https://www.google.com/maps?q=${editingTurno.clockInLat},${editingTurno.clockInLng}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-blue-600 hover:underline"><HiLocationMarker className="w-3 h-3" /> Ubicación inicio</a>
                )}
                {editingTurno.clockOutLat != null && (
                  <a href={`https://www.google.com/maps?q=${editingTurno.clockOutLat},${editingTurno.clockOutLng}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-blue-600 hover:underline"><HiLocationMarker className="w-3 h-3" /> Ubicación fin</a>
                )}
