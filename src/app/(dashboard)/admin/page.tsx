...
import { HiUserGroup, HiOfficeBuilding, HiGlobeAlt, HiLocationMarker } from "react-icons/hi";
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/reportes?inicio=${inicio}&fin=${fin}&zona=ALL`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div><h2 className="text-2xl font-bold text-gray-900 dark:text-white">Dashboard Administrador</h2>
          <p className="text-gray-500 dark:text-[#A0AEC0]">Vista global — Todas las zonas</p>
          <p className="text-gray-500 dark:text-[#A0AEC0] text-sm">{format(ahora, "MMMM yyyy", { locale: es })}</p></div>
      ...
      <div><h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Detalle por operador</h3>
        <DataTable columns={columns as never} data={detalleVista as never} searchable searchPlaceholder="Buscar operador..." /></div>
    </div>
  );
}
