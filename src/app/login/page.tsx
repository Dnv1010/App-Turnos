...
function LoginForm() {
  ...
  const handleCredentials = async (e: React.FormEvent) => {
    ...
    try {
      // Llamar a API de login personalizada
      ...
      if (!response.ok) {
        setError(data.error || "Email o PIN incorrectos");
        return;
      }
  ...
  const handleGoogle = async () => {
    try {
      ...
      if (error) {
        console.error('[login] Error OAuth:', error);
    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-white dark:bg-[#1A2340] rounded-2xl shadow-2xl dark:shadow-black/40 p-8 border border-gray-200 dark:border-[#3A4565] dark:text-white">
        ...
        <form onSubmit={handleCredentials} className="space-y-5">
          ...
          {error && <div className="bg-red-50 text-red-700 text-sm px-4 py-3 rounded-lg dark:bg-red-900/20 dark:text-red-400">{error}</div>}
      ...
      <p className="text-center text-primary-200 text-xs mt-6">Bia Energy - Sistema de Gestión de Turnos v2.0 (Supabase)</p>
    </div>
  );
}
