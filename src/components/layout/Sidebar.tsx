...
const navItems: NavItem[] = [
  ...
  { label: "Dashboard Líder de Zona", href: "/coordinador", icon: HiUserGroup, roles: ["COORDINADOR"] },
  { label: "Turnos Equipo", href: "/coordinador/turnos", icon: HiCollection, roles: ["COORDINADOR"] },
  { label: "Fichaje Líder de Zona", href: "/coordinador/turno-coordinador", icon: HiClock, roles: ["COORDINADOR"] },
  ...
  { label: "Dashboard Supply", href: "/supply", icon: HiHome, roles: ["SUPPLY"] },
  { label: "Turnos Equipo", href: "/supply/turnos", icon: HiCollection, roles: ["SUPPLY"] },
  { label: "Mi Equipo", href: "/supply/equipo", icon: HiUserGroup, roles: ["SUPPLY"] },
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={onClose}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-primary-50 text-primary-700 dark:bg-bia-teal/10 dark:text-bia-teal" : "text-gray-600 dark:text-bia-muted hover:bg-gray-100 dark:hover:bg-bia-navy-500 hover:text-gray-900 dark:hover:text-bia-teal"}`}>
                <item.icon className={`h-5 w-5 ${isActive ? "text-primary-600 dark:text-bia-teal" : "text-gray-400 dark:text-bia-muted"}`} />
                {item.label}
              </Link>
            );
          })}
